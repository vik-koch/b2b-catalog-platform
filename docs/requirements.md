...
| FR-ADM-13    | What the shop presents rather than stocks stays the shop's however an area is owned (FR-ADM-10): a product's publication, and how its categories are named for display, pictured and arranged. A price list stays the shop's to name, add and remove — only the key the external system addresses it by is frozen, because that key is how its prices arrive. |
| FR-ADM-14    | Only an admin may change an ownership setting (FR-ADM-10); it takes effect without a deploy, and every change to it — and to maintenance mode (FR-ADM-04) — is recorded with who made it and shown in the admin panel. |
