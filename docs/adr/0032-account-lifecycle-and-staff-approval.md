...
leaves no trace and the same address can retry endlessly unnoticed); and hard
...
`pending`; approval by staff sets `active` together with `approvedAt`,
...
- (−) Every future guard, and any query that lists "users", must remember that
  `anonymized` rows exist and are not customers. Staff-facing counts already
  filter by role; they will need to filter by status too.
- (−) An unapproved registration occupies its email address indefinitely. There is
  no expiry sweep; if pending rows pile up, that is a staff workflow signal, and
  a cleanup job can be added later without touching this model.
