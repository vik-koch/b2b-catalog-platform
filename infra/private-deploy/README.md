...
3. **Secrets & variables** (private repo → Settings → Secrets and variables → Actions):
   ...
   | `MAIL_HOST` / `MAIL_PORT` / `MAIL_FROM` / `MAIL_STAFF_TO` | SMTP provider + addresses                                                                                                                                                                                                                                                                                                                                                                 |
   | `MAIL_ADMIN_TO`                                           | Where the shop's admin is written to: a sync run waiting to be applied, a sync that broke. Not `MAIL_STAFF_TO` — these mails link into the admin panel                                                                                                                                                                                                                                    |
   ...
   | `MAIL_OPS_TO`                                             | Optional; where ops alerts go (disk, backup failures, error spikes) and a second copy of the sync failure mails. The operator — **not** `MAIL_STAFF_TO`, which is where customer enquiries land, and not instead of `MAIL_ADMIN_TO`, which gets those failures either way. Alerts reuse the `MAIL_*` SMTP account, so no second provider is needed. Renamed from `ALERT_EMAIL` in v1.10.0 |
