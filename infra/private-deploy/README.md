...
3. **Secrets & variables** (private repo → Settings → Secrets and variables → Actions):
   ...
   | `ALERT_EMAIL`                                             | Optional; where ops alerts go (disk, backup failures, error spikes). The operator — **not** `MAIL_STAFF_TO`, which is where customer enquiries land. Alerts reuse the `MAIL_*` SMTP account, so no second provider is needed |
