...
3. **Secrets & variables** (private repo → Settings → Secrets and variables → Actions):
   ...
   | `MAIL_OPS_TO`                                             | Optional; where ops alerts go (disk, backup failures, error spikes) and a second copy of the sync failure mails. The operator — **not** `MAIL_STAFF_TO`, which is where customer enquiries land. Alerts reuse the `MAIL_*` SMTP account, so no second provider is needed. Renamed from `ALERT_EMAIL` in v1.10.0 |
